...
from modules.scene import Scene, GaussianModel
from modules.gaussian_renderer import render
...
def prepare_output_and_logger(args) -> 'SummaryWriter':
...
def training_report(tb_writer:'SummaryWriter', iteration:int, Ll1:float, loss:Tensor, l1_loss:Callable, elapsed, testing_iterations:List[int], scene:Scene, renderFunc:Callable, renderArgs:Tuple):
    if tb_writer:
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{7}f}", "n_pts": gaussians.get_xyz.shape[0]})
