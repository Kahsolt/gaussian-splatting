...
def train(args:Namespace, hp:HyperParams):
    ...
    for steps in range(start_steps, hp.iterations + 1):
        ...
        # Render
        render_pkg = render(gaussians, viewpoint_cam, scene.random_background() if hp.random_background else scene.background, steps=steps, model_path=scene.model_path)
