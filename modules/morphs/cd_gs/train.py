...
def network_gui_handle(render_func:Callable, scene:Scene, steps:int):
    ...
    while network_gui.conn != None:
        try:
            ...
            if custom_cam != None:
                rendered_set = render_func(scene.gaussians, custom_cam, scene.background, scaling_modifer)['render']
                rendered_mix = mix_image(rendered_set)
                net_image_bytes = memoryview((torch.clamp(rendered_mix, min=0, max=1.0) * 255).byte().permute(1, 2, 0).contiguous().cpu().numpy())
...
def train(args:Namespace, hp:HyperParams):
    ...
    for steps in range(start_steps, hp.iterations + 1):
        ...
        image = mix_image(rendered_set)                     # [C=3, H=545, W=980]
        ...
        with torch.no_grad():
            ...
            if steps % 10 == 0:
                pbar.set_postfix({'loss': f'{loss_ema_for_log:.7f}', 'n_pts': gaussians.n_points})
                pbar.update(10)
            ...
            if steps % 100 == 0:
                ...
                rendered_cat = torch.cat([torch.cat(rendered_set, -1), image, gt_image], -1)
            ...
            if steps in hp.test_iterations:
                ...
                for split, cameras in validation_configs.items():
                    ...
                    for idx, viewpoint in enumerate(cameras):
                        ...
                        rendered = torch.clamp(mix_image(render_pkg['render']), 0.0, 1.0)
