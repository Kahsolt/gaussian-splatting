...
from modules.gaussian_renderer import render, DGR_PROVIDER, ImageState, GaussianModel
...
def render_set(model_path, name, iteration, views, gaussians:GaussianModel, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if DGR_PROVIDER == 'ours':
            save_image(gaussians.importance_activation(render_results['importance_map']), os.path.join(importance_path, '{0:05d}'.format(idx) + ".png"), normalize=True)
        if DGR_PROVIDER == 'ours-dev':
            img_state: ImageState = render_results["img_state"]
            save_image(img_state.final_T, os.path.join(finalT_path, '{0:05d}'.format(idx) + ".png"))
            plt.clf()
            ...
            save_image(render_results['depth_map'] / render_results['depth_map'].max(), os.path.join(depth_path, '{0:05d}'.format(idx) + ".png"))
            save_image(render_results['weight_map'], os.path.join(weight_path, '{0:05d}'.format(idx) + ".png"))
